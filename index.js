...
Stay motivated and keep learning! 🚀📚
       `;
      await client.sendMessage(gid, leaderboardMessage, {
  ...
  //           GROUP_IDs.map((id) => readFromRedis(id))
  //         );
  ...
  //         for (let index = 0; index < GROUP_IDs.length; index++) {
  //           const gid = GROUP_IDs[index];
  //           await client.sendMessage(gid, media, {
  //             caption: "Refer to this code.",
  //           });
  //         }
  //         // const formattedCode = `\`\`\`\n${currentQuiz?.code}\n\`\`\``
  ...
  //           options.findIndex(
  //             (ops) => ops === answer.slice(0, 1).toLocaleLowerCase()
  //           ),
  //           gid
  //         );
  //       }
    ...
    try {
      ...
      for (let index = 0; index < GROUP_IDs.length; index++) {
        ...
        const message = await client.sendMessage(
          ...
          new Poll(
            current.Question,
            [current["Option 1"], current["Option 2"], current["Option 3"]],
            {
              allowMultipleAnswers: false,
            }
          )
        ...
        await saveAnswer(
          message.id._serialized,
          options.findIndex(
            (ops) => ops === current.Answer.slice(0, 1).toLocaleLowerCase()
          ),
          gid
        );
      }
      await moveQuizStatusToDone(current.ID);
    } catch (err) {
      console.log(err);
    }
  });
